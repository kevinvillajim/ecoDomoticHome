			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Ubicaciones del mapa del sitio - Eco Domotic Home</title>
					</xsl:when>
					<xsl:otherwise>
						<title>Mapa del sitio XML - Eco Domotic Home</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="./index.htmlsitemap_index.xml">&#8592; Índice del mapa del sitio</a>							</p>
